...
int getint(){int t; scanf("%lld",&t); return t; }
int getch(){char c; scanf("%c",&c); return (int)c; }


void putint(int a){ printf("%lld",a);}
...
__attribute((destructor)) void after_main(){
  for(int i=1;i<_sysy_idx;i++){
    fprintf(stderr,"Timer@%04lld-%04lld: %lldH-%lldM-%lldS-%lldus\n",\
  ...
  fprintf(stderr,"TOTAL: %lldH-%lldM-%lldS-%lldus\n",_sysy_h[0],_sysy_m[0],_sysy_s[0],_sysy_us[0]);
}  
